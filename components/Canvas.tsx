  ...
  const updatePhysics = (width: number, height: number) => {
    ...
    state.balls.forEach(ball => {
        ...
        for (let i = 0; i < localVertices.length; i++) {
            ...
            // Note: For custom hulls generated by Monotone Chain, winding order matters.
            // Assuming CCW winding (standard for convex hulls), normal (-edge.y, edge.x) points LEFT of vector, 
            // which is INSIDE if walking perimeter CCW.
            
            let edgeNormal = normalize({ x: -edge.y, y: edge.x }); 
            
            // Verify normal points towards center (0,0) roughly?
            // If center is inside, dot(center - p1, normal) should be > 0
            // center is 0,0 relative here.
